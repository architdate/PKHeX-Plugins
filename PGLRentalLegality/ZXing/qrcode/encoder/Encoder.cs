...
namespace com.google.zxing.qrcode.encoder
{
    ...
    public static class Encoder
    {
        ...
        public static void Encode(string content, ErrorCorrectionLevel ecLevel, System.Collections.Hashtable hints, QRCode qrCode)
        {
            string encoding = (string) hints?[EncodeHintType.CHARACTER_SET] ?? DEFAULT_BYTE_MODE_ENCODING;
        ...
        /// <summary> the code point of the table used in alphanumeric mode or
        ...
        /// </summary>
        public static Mode ChooseMode(string content, string encoding = null)
        {
        ...
        internal static void TerminateBits(int numDataBytes, BitVector bits)
        {
            ...
            {
                bits.AppendBits(i % 2 == 0 ? 0xec : 0x11, 8);
            }
            if (bits.Size() != capacity)
            {
        ...
        internal static void InterleaveWithECBytes(BitVector bits, int numTotalBytes, int numDataBytes, int numRSBlocks, BitVector result)
        {
            ...
            for (int i = 0; i < maxNumDataBytes; ++i)
            {
                ...
                {
                    ByteArray dataBytes = ((BlockPair) b).DataBytes;
                    if (i < dataBytes.Size())
                    {
            ...
            for (int i = 0; i < maxNumEcBytes; ++i)
            {
                foreach (object b in blocks)
                {
                    ByteArray ecBytes = ((BlockPair) b).ErrorCorrectionBytes;
                    if (i < ecBytes.Size())
                    {
